...
# ── DRIVE CONFIG ─────────────────────────────────────────────────
# Reads DATA_DRIVE from environment (set by the .bat launcher).
# Falls back to E: if run standalone, so this never silently points
# at a stale/wrong drive again. Override with: set DATA_DRIVE=D:
DATA_DRIVE = os.environ.get('DATA_DRIVE', 'E:')
DB_PATH = os.path.join(DATA_DRIVE, r'\RealSlate\realslate_core\realslate.db')
OUTPUT_DIR = os.path.join(DATA_DRIVE, r'\RealSlateOS\data\raw')

if not os.path.exists(DB_PATH):
    print(f"FATAL: realslate.db not found at {DB_PATH}")
    print(f"  DATA_DRIVE={DATA_DRIVE} — set the correct drive letter via 'set DATA_DRIVE=X:' before running,")
    print(f"  or check that realslate_core lives at {DATA_DRIVE}\\RealSlate\\realslate_core\\")
    sys.exit(1)
...
if __name__ == '__main__':
    try:
        ...
        print(f"FATAL: parse_and_export failed: {e}")
        sys.exit(1)
